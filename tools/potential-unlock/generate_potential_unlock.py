...
# ── Per-item attributes to copy from source → unlocked ──────────────────────
# Tier/slot/role invariants (boundType, tradable, rareGrade, linkEnchantId,
# linkPassivityCategoryId, combatItemType, etc.) flow in from the
# equipment-item-standard package definition chosen via `resolve_tier_def()`,
# so they are intentionally NOT copied here. DSL equipment defaults cover
# other omitted attrs (searchable, destroyable, linkCrest/Customizing/PetAdult/
# PetOrb/SkillId, sortingNumber, slotLimit, itemUseCount, maxDropUnit, maxStack,
# coolTime/Group, conversion, useOnlyTerritory, requiredGuildMaster, changeLook,
# extractLook, artisanable, requiredEquipmentType, dropSound, and conditional
# defaults icon/dropSilhouette/equipSound/requiredClass).
COPY_ATTRS = [
    # identity + tier-independent metadata
    "name", "combatItemSubType", "category", "rank", "level", "requiredLevel",
    # per-item lists
    "linkPassivityId", "requiredClass",
    # economy
    "buyPrice", "sellPrice", "extractRatio",
    # visuals + links (item-specific)
    "linkLookInfoId", "icon", "defaultValue", "itemLevelId",
    "linkMaterialEnchantId", "changeEnchantFxId",
    # conservative keeps (not in DSL default table)
    "changeColorEnable", "divide",
]

# ── rareGrade → package tier prefix ─────────────────────────────────────────
TIER_BY_GRADE = {
    "4": "HighTier",
    "3": "MidTier",
    "2": "LowTier",
...
# ── combatItemSubType → package definition suffix ───────────────────────────
DPS_TANK_WEAPONS = {
    "dual", "lance", "twohand", "axe", "circle", "bow",
    "blaster", "shuriken", "glaive",
}
HEALER_WEAPONS = {"staff", "rod"}


def resolve_tier_def(rare_grade: str, sub_type: str) -> str:
    """Map (rareGrade, combatItemSubType) → equipment-item-standard definition."""
    tier = TIER_BY_GRADE.get(rare_grade)
    if tier is None:
        raise ValueError(f"Unsupported rareGrade for unlock gear: {rare_grade}")

    if sub_type in HEALER_WEAPONS:
        return f"{tier}WeaponHealerItem"
    if sub_type == "chain":
        return f"{tier}ChainWeapon"
    if sub_type == "gauntlet":
        return f"{tier}GauntletWeapon"
    if sub_type in DPS_TANK_WEAPONS:
        return f"{tier}WeaponDPSTankItem"
    if sub_type.startswith("body"):
        return f"{tier}ChestItem"
    if sub_type == "handMail":
        return f"{tier}HandMailItem"
    if sub_type == "handLeather":
        return f"{tier}HandLeatherItem"
    if sub_type == "handRobe":
        return f"{tier}HandRobeItem"
    if sub_type.startswith("feet"):
        return f"{tier}BootsItem"
    raise ValueError(f"Unknown combatItemSubType '{sub_type}' for rareGrade {rare_grade}")
...
def generate_gear_spec(
    ...
    # Resolve the equipment-item-standard definition for each source item up
    # front so we can emit a minimal `imports` block listing only what's used.
    item_tier_defs: dict[int, str] = {}
    for entry in entries:
        source_id = entry["source_id"]
        item = items_data[source_id]
        rare_grade = item.get("rareGrade", "")
        sub_type = item.get("combatItemSubType", "")
        item_tier_defs[source_id] = resolve_tier_def(rare_grade, sub_type)

    used_defs = sorted(set(item_tier_defs.values()))
    ...
    lines = [
        ...
        "#",
        "# Each unlocked item extends the matching equipment-item-standard tier",
        "# definition, which owns every tier-wide invariant (boundType, tradable,",
        "# dismantlable, rareGrade, linkEnchantId, linkPassivityCategoryId,",
        "# combatItemType, guildWarehouseStorable, storeSellable, etc.). DSL",
        "# equipment defaults cover the remaining boilerplate.",
        "",
        "spec:",
        '  version: "1.0"',
        "  schema: v92",
    ...
    for entry in entries:
        ...
        tier_def = item_tier_defs[source_id]

        lines.append(f"    # --- Unlocked {display_name} ({category}) ---")
        lines.append(f"    - $extends: {tier_def}")
