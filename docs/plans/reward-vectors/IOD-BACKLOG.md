...
- **RV-01 Early-progression token item.** Allocate item id (find_free_ids), author ItemTemplate on the currency-token archetype (D §4.2): `boundType=Loot`, `tradable=false`, NO_COMBAT, high maxStack; name/tooltip strings both legs. Gate: item exists, drops, stacks, cannot trade.
  ...
  - The constant belongs in **`packages/iod-tokens/index.yml`** (alongside `KUGAI_TOKEN`), not in `packages/item-ids`, which is tool-generated and explicitly excludes ids named by another package. `iod-tokens` is already registered in `datasheetlang.yml`.
  ...
  - `boundType: Loot` (R21) is a deliberate divergence from item 72 and from all three existing project tokens, which ship `boundType: None` with `tradable: false`. Divergence-log it when authored.
...
- **RV-03 Token threading.** Add RV-01 payouts to: zone quest compensations, repeatables (1341/1390/1334 band), daily pool quests (RV-11), Sorcha/Karasha clears (small), exploration caches (RV-20), treasure chests (RV-22). Tokens are granted as ordinary `Item` rows in QuestCompensation and as ECompensation drops; quest rewards cannot grant currencies directly (D §4.1). Gate: every vector demonstrably pays tokens in a live pass.
  - Measured 2026-07-28, zone-quest leg: **35 live IoD zone quests** (category Normal, enabled and reachable), of which 3 are repeatables (1334, 1341, 1390) that belong to RV-08, leaving **32 one-time zone quests**. The 11 sentinel-disabled quests (1306, 1307, 1308, 1310, 1318, 1323, 1343, 1344, 1385, 1386, 1389) are inert and take no token.
