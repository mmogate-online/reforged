...
Rulings added 2026-07-28 (planning session over the research wave in §4.10, all user-confirmed):

| # | Ruling |
|---|--------|
| R16 | The wave folds into the OPEN patch 002, not a new patch: every affected row exists only in the dirty working tree, so a replay simply never writes them and no delete op is needed |
| R17 | Family A (`EnchantData` / `EnchantDecompositionData`) KEEPS paying feedstock: players recover feedstock when dismantling gear. Only the output identity collapses to one item; the existing grade ladder (`resultItemAmount` 4/8/24/48) stays |
| R18 | Surviving feedstock id is 94101. Tiers 94102 to 94112 are retired, but their `ItemTemplate` rows STAY resident and unreferenced: deleting them would create about 4,000 dangling item references, and `Gacha` and `LimitedGacha` are proven to validate item ids at load |
| R19 | Migrate versus delete is decided per family. MIGRATE the mechanic families (`MaterialEnchantData`, `EnchantData`, `DecompositionData`). DELETE the faucet families (`ECompensation`, `EventMatching`, `Gacha`, `ItemConversion`, `AchievementList`, `StackAttendanceEvent`, the dead vanilla feedstock shop). DELETE `ItemMixData`'s ladder outright, because flattening its ids would turn every rung into a recipe that consumes N and returns 1 |
| R20 | The R10 zone-quest XP cap is measured against the MEDIAN story quest XP of the same level bracket, not the peak |
| R21 | **RESTATED 2026-07-30 (user), replacing the original.** Tokens are restricted by `tradable: false` **and** `guildWarehouseStorable: false`, never by `boundType`. `boundType` is for EQUIPMENT the character wears; a token is a consumable currency, and what it must not do is move between PLAYERS, which those two flags are exactly what gate. `warehouseStorable: true` stays: the personal bank is same-account only, so it moves nothing between players. Applies to the whole reserved token band and is enforced by `tools/dc-restore/audit_item_references.py`. THE ORIGINAL RULING WAS ALSO UNSHIPPABLE: it asked for `boundType: Loot`, and `WorldServer.exe` refuses any `boundType` on a stackable item (`stackable item cannot specify boundType [ItemTID=95217]`), which a currency must be. Consequence to carry: seals pool per ACCOUNT via the personal bank, so RV-02 prices against per-account income. No divergence row is owed for the bind behaviour any more, since the token now matches the family; the log keeps one recording the overturn |
| R22 | A token paid into an `itemBag="class"` quest is authored as 12 duplicate rows, one per class: that is the only idiom the corpus uses |

Rulings added 2026-07-30 (token architecture session, all user-confirmed):

| # | Ruling |
|---|--------|
| R23 | **Story quests pay the early-progression token.** Their exclusion was never ruled: it lived only in the §2 role table and RV-03's target list, and it contradicted both R1 ("every vector pays some of it") and framework `02 §1`, whose Lane A row (story quests, side quests, NPC dailies) lists early-progression tokens as a Lane A reward. The marginalisation is measurable: after the patch-002 trimming waves, story quests 1303, 1304, 1305, 1315 and 1331 pay exp and gold and NOTHING else, leaving the spine the only lane with no tangible drop. Tokens are priced by EFFORT, not by lane (R26): a story quest and a zone quest of similar length pay similarly, and lane identity stays where the framework puts it, XP for story versus gold and gear for zone. Amends §2 and RV-03 |
| R24 | **Tokens are NOT bankable. RESTATES R21 a second time.** `warehouseStorable: false` joins the existing `tradable: false` and `guildWarehouseStorable: false`. R21 kept the personal bank open because it read the threat as player-to-player transfer; the real leak is one player farming the same one-time content on several characters and pooling into a main. Closing the bank welds tokens to the character that earned them and removes the alt loophole at the source. Verified shippable BEFORE ruling, because the previous two token rulings both hit loader traps: `warehouseStorable="False"` occurs 4,607 times in `ItemTemplate.xml`, on 1,653 stackable items and 642 `NO_COMBAT` items, and vanilla item **72 `dungeon_coin` ("Challenge Receipt")** already ships the exact target shape (`NO_COMBAT`, `tradable=False`, `warehouseStorable=False`, `guildWarehouseStorable=False`, `boundType=None`, stackable 1000). Our four tokens 95214 to 95217 are the divergence from vanilla, not this change. Author the value LOWERCASE `false`: mixed-case booleans are what broke the `ContinentData` client sync in July. Consequence, inverting R21's: RV-02 prices against PER-CHARACTER income |
| R25 | **Two band-wide currencies, not one per region.** The early-progression token covers ALL sub-60 content in every zone; a separate endgame token covers level cap. Matches framework `02 §5` and `03 §3b-i`, where the token is earned across early content generally and the leveling-scroll conversion is one spend among several, not the currency's identity. Consequences: the item stops being Island of Dawn scoped (R27), and the constant `IOD_PROGRESSION_TOKEN` in `packages/iod-tokens/index.yml` moves to a new `progression-tokens` package as `EARLY_PROGRESSION_TOKEN`, leaving `iod-tokens` to Kugai. Blast radius measured: 130 references across specs 28, 29, 30, 39 and 40, plus the package. Two band-wide tokens is the rule-of-two that justifies the package |
| R26 | **Token amounts follow an effort-priced tier ladder with a 150 ceiling. There is NO level-band curve.** A per-band curve was proposed and dropped. Its defensive rationale died with R24, since alts can no longer feed a main, and a steep curve would make the 46-60 band the only efficient farm, defeating the token's stated purpose of keeping EARLY zones populated and worth a synced-down vet's time (`02 §5`, `03 §3b-i`). Price by effort per activity instead: framework reward parity is time-based (four hours of early content pays roughly like four hours of mid content), and a gentle curve emerges on its own because higher-level quests take longer. One table keyed by content type (capstone / story / long zone / short zone / repeatable), ceiling 150 on a capstone. The actual numbers come from a measurement pass counting every token-paying vector in scope, batch-0 style, NOT from a strawman. What it replaces: 35 quests at 1 token each, 35 tokens for the whole island, which has no price resolution at all |
| R27 | **Presentation.** Name: **"Valkyon Commendation"**, replacing "Dawn Seal", chosen for the one institution that spans the whole sub-60 band; `Valkyon <noun>` is already the game's issue-prefix (19 item names) and "Commendation" has ZERO item-name collisions, while "Vanguard" is taken twice over by `100 Vanguard Initiative Credits` and the live Vanguard Request system. Icon: `Icon_Items.mysterypaper6_Tex`. The paper family is now allocated: **5 = dungeon tokens** (95214, 95215, 95216, plus vanilla 73 and 55672), **6 = early progression**, **8 = endgame** (vanilla item 72 only). `mysterypaper5_Tex` was the first choice and is rejected: it would put four visually identical currencies in one bag. Drop presentation leaves the chest default for the paper pair, `dropSilhouette: Item_Drop.SM.Recipe_SM` (4,018 uses) and `dropSound: InterfaceSound.Drop_ItemCUE.Drop_ScrollCue` (5,557 uses); both are inert until RV-03's drop vectors ship and are set now only because the file is open. Strings follow the KB contract in `entities/item-system.md`: `string` is plain text with no formatting, `toolTip` takes `$BR` and entity-encoded `<font color='#HEX'>`, blue `#2478FF` for acquisition, red `#FF0000` for the bind warning, amber `#FFBB00` for the temporary no-vendor-yet line that comes out when RV-02 ships. Item id 95217 does NOT change, so nothing a tester holds breaks; the name renders client-side, so it needs a client publish |

## 2. Vector roles (the design contract)

| Vector | Role | Framework |
|---|---|---|
| Story quests | High XP pacing, story progression, minimal power, **early-prog tokens (R23)** | `02 §3a` Lane A baseline + `02 §1` Lane A rewards |
...
Wave 1 is CLOSED. Its items shipped inside the open patch 002, gated, and deployed to dev as client
`0.1.0-dev.42`. Live validation was **deliberately skipped** by user ruling 2026-07-31: the remaining
deltas at that point were tooltip text, which does not warrant a restart and test cycle.

| Item | State | Where it lives now |
|---|---|---|
| RV-01 token item | **DONE** | `specs/patches/002/39`. Item 95217 "Valkyon Commendation" |
| RV-03 token threading | **PARTIAL** | Story, zone and repeatable legs done: 63 quests pay. Outstanding legs are daily pool (RV-11), Sorcha and Karasha clears (RV-04), exploration caches (RV-20) and treasure chests (RV-22). Each waits on content that does not exist yet, not on this item |
| RV-07 zone quest XP pass | **DONE** | `002/40` plus amendments to `002/28`, `29`, `30`. 26 of 35 quests clamped, pool 58,200 to 24,740 |
| RV-08 repeatable pass | **DONE for what exists** | 1334 and 1341 keep their v31 level caps and pay 5 tokens per completion |
| RV-23 framework authoring | **DONE** 2026-07-26 | Framework `11-region-progression.md` |
| RV-26 loot correction | **PARTIAL** | The feedstock slice is done (`002/38`, `002/41`). The rest of R13 is untouched: MWA still drops on trash rather than named elites and dungeon clears, and dyad drops are not capped at rare structures |
| RV-28 feedstock flattening | **DONE** | `002/34`, `35`, `37`, `38`, `41`. All reachable enchanting consumes 94101 |
| PROBE-9 | **ANSWERED by research, no experiment needed** | Dossier §4.9: `ECompensation` carries no level-conditional terms. The decay is a pure Mystel Proxy feature |
| Everything else | **NOT STARTED** | RV-02, 04, 05, 06, 09 to 22, 24, 25, 27, 30 to 33 |

**The wave-1 planning documents were retired on 2026-07-31**, after a check found their content
duplicated at the point of use. `IOD-WAVE1-PLAN.md`, `IOD-WAVE1-BATCH0-MEASUREMENTS.md` and
`IOD-WAVE2-TOKEN-ARCHITECTURE.md` are deleted, 1,827 lines in total. Where each thing went:

| What the plan held | Where it lives now |
|---|---|
| Spec ownership map (`questCompensations: upsert` is replace-all, so exactly one spec states each quest) | `002/40` and `002/42` headers, both stating the complete map. **Still true**: verified against the current DSL docs, which say the compensation families have no granular collection membership |
| The token ladder | Story tiers in `002/42`'s header, zone tiers in `002/40`'s. The 1,182-per-character-clear income anchor is in framework `99 #02-progression-lanes` |
| Batch-0 XP measurements | Restated per quest in the `002/28`, `29`, `30`, `40` headers |
| The `AchievementList` narrowing warning and the four sync descriptor shapes | `config/sync-config.yaml`, more completely than the plan had them |
| Both boot failures and the sum-to-1 collection taxonomy | `tools/dc-restore/audit_item_references.py`, as executable gates rather than prose, and in the DSL's own `reference/collection-membership.mdx` |
| Operator and database questions | Section 7 of this document |
| The wave's own narrative about DSL gaps | **Stale and discarded.** Every gap it described was delivered: `E573` plus `normalize`, collection membership, `inProgress` clearing, regional-variant targeting, the `stackAttendanceEvent` entity. The plan's reasoning about working around them no longer describes any real constraint |

**Wave-1 regression checklist**, the residue of the acceptance pass that was skipped. Not owed now;
worth walking if the enchant economy ever behaves oddly.

- Enchanting at a leveling tier and an endgame tier consumes 94101 and Masterwork Alkahest, nothing else.
- Dismantling infusion fodder yields 16 / 48 / 96 on weapon and body by grade, and 8 / 24 / 48 on arm and leg.
- Island of Dawn trash drops no feedstock; Vekas `13,1001` and Kugai `13,1004` still do (the classic carve-out).
- The feedstock tier-up right-click affordance is gone from 94101.
- A Vanguard Request at level 11 or above pays no tier feedstock.
- Kugai's Crest still opens its exchange and its 94101 row still sells.

### A. Token spine

- **RV-01 Early-progression token item. DONE, spec `002/39`.** Allocate item id (find_free_ids), author ItemTemplate on the currency-token archetype (D §4.2): `boundType=None`, `tradable=false`, `warehouseStorable=false`, `guildWarehouseStorable=false` (R21 as restated, then R24), NO_COMBAT, high maxStack; name/tooltip strings both legs. Gate: item exists, drops, stacks, cannot trade.
  - Measured 2026-07-28: **id 95217** is the next free slot in the project's own reserved token band (`packages/dungeon-tokens/index.yml` documents 95214-95313; 95214/95215/95216 are taken). Verified free three ways: `find_free_ids`, `tools/item-ids/gen_item_ids.py check`, and `item_sources 95217` = 0 sources across every scanned family. Hold `95217..95226` if RV-04's two dungeon tokens should sit adjacent.
  - The constant was placed in `packages/iod-tokens/index.yml` (alongside `KUGAI_TOKEN`), not in `packages/item-ids`, which is tool-generated and explicitly excludes ids named by another package. **SUPERSEDED 2026-07-30 by R25:** the token is band-wide, so it moves to a new `progression-tokens` package as `EARLY_PROGRESSION_TOKEN`. Register the new package in `datasheetlang.yml` or every importing spec fails with `Unknown package reference`.
  - Attribute shape: the token is **Kugai's Crest (95216) minus `MEDAL_USEABLE`, minus `usedSound`, minus the `[Right-click]` tooltip line**, with `combatItemType: NO_COMBAT`. Copy-from is the `_DungeonToken` definition plus `items: upsert:` block in `specs/patches/002/14-dungeon-tokens.yaml` (its menu/BuyList wiring is the MEDAL_USEABLE shape R11 rejects: take the item half only). No token archetype package exists yet; a third token is the rule-of-two point at which promoting one is justified.
  - **Shipped shape, after two overturns.** `boundType: Loot` (original R21) was unshippable: the world server refuses any `boundType` on a stackable item. R21 as restated then kept the personal bank open; R24 closes it. Final shape is vanilla item 72 `dungeon_coin` exactly: `boundType: None`, `tradable: false`, `warehouseStorable: false`, `guildWarehouseStorable: false`. No divergence row is owed for bind behaviour, since the token now matches the base game's own currency archetype; the log keeps the row recording the overturns.
  - Presentation per R27: name **"Valkyon Commendation"**, icon `Icon_Items.mysterypaper6_Tex`, `dropSilhouette: Item_Drop.SM.Recipe_SM`, `dropSound: InterfaceSound.Drop_ItemCUE.Drop_ScrollCue`. The shipped wave-1 values (name "Dawn Seal", icon `drop_medal_01_Tex`, chest silhouette and chest drop sound, Island-of-Dawn tooltip) are all superseded.
  - **Sync trap:** `migrate.py detect_entities()` anchors `ENTITY_KEY_PATTERN` at column 0, so an inline `strings:` block nested under `items:` never registers the `itemStrings` key. Author the name and tooltip in a **top-level `itemStrings: upsert:` block**, or verify the migrate run logs `StrSheet_Item` among the synced entities, otherwise the client ships a nameless item.
- **RV-02 Early-prog vendor at Tower Base.** A PHYSICAL villager NPC (R11; this is the project's first end-to-end authored NPC token shop, canon baseline: NPC 59,1955 "Major Milestone" MedalStore, D §4.2) carrying a Merchant/MedalStore menu whose BuyList tab is item-priced in RV-01 (`NeedMedalItemId`, D §4.2) plus matching ItemMedalExchange rows. Phase-1 catalog per `03 §3b-i`, sized by the reward-parity rule: enchant material bundles, uncommon crystals, consumables, a first cosmetic. Leveling-scroll conversion deferred (needs the level-sync era). Gate: purchase works live at correct prices.
  - **SCOPING RULING 2026-07-31 (user).** The quartermaster network is real and band-wide, but it is built REGION BY REGION. Wave 2 places exactly ONE, at Tower Base, and touches no other hub. Item 95217's tooltip says "quartermasters in major towns and outposts", plural, which is aspirational until the network fills in; that is the accepted trade-off recorded when the line was authored, not an oversight, and no string needs editing as each new quartermaster lands.
  - Build plan: `IOD-WAVE2-PLAN.md`. Canon shape fully measured there from NPC `59,1955`; the only structural difference between a right-click shop and an NPC shop is `villagerMenuItems` (binds an item) versus `villagerMenus` (binds an NPC).
- **RV-03 Token threading. PARTIAL: story, zone and repeatable legs DONE.** Add RV-01 payouts to: **story quest compensations (R23, added 2026-07-30)**, zone quest compensations, repeatables (1341/1390/1334 band), daily pool quests (RV-11), Sorcha/Karasha clears (small), exploration caches (RV-20), treasure chests (RV-22). Tokens are granted as ordinary `Item` rows in QuestCompensation and as ECompensation drops; quest rewards cannot grant currencies directly (D §4.1). Gate: every vector demonstrably pays tokens in a live pass.
  - Measured 2026-07-28, zone-quest leg: **35 live IoD zone quests** (category Normal, enabled and reachable), of which **2 are repeatables (1334 and 1341)** that belong to RV-08, leaving **33 one-time zone quests**. _(CORRECTED 2026-07-31: this line read "3 are repeatables (1334, 1341, 1390)". Quest 1390's header is `1회성`, one-time; only 1334 and 1341 carry `반복`. Nothing shipped wrong, because the token ladder priced 1390 as a one-time zone quest at 20. Verify repeatability from the quest header, never from a list.)_ The 11 sentinel-disabled quests (1306, 1307, 1308, 1310, 1318, 1323, 1343, 1344, 1385, 1386, 1389) are inert and take no token.
  - **Every IoD quest reward lives in `QuestCompensationData_13.xml` regardless of which HZ the giver stands in**, so every op uses `huntingZoneId: 13` (including the HZ 64 Berlon chain and the Sorcha pair).
  - Bag split, which is what sets the cost: **12 quests already `allpay`** (token appends as one row), **15 with no `itemBag` at all** (add `itemBag: "allpay"` plus one row), and **8 `class` bags** (1319, 1322, 1324, 1325, 1326, 1330, 1332, 1333) which take **12 duplicate token rows each per R22**, so 96 extra rows. In-file precedent for the 24-row shape is neighbouring quest 1310; 13 bags corpus-wide already sit at exactly 24 rows and the ceiling is 104.
  - `questCompensations` `upsert` is **REPLACE-ALL**: each op must restate the complete block, every `exp`/`gold`/`itemBag` value and every surviving row.
  - Note 1351 and 1352 are duplicate quests sharing reward table RT-36: threading a token into both doubles that payout.
...
- **RV-26 Loot table correction pass (R13). PARTIAL: feedstock slice DONE, MWA and dyad legs NOT started.** Audit the patch-002 merged IoD table (specs 001/20 + 002/17): remove all direct feedstock drops; move MWA off trash mobs onto named uniques/elites, dungeon clears, and the faction dailies; cap dyad drops at rare structures (remove any superior structures or finished dyads); keep the infusion-fodder grade spread and the entire classic v31 layer byte-untouched. Document the per-drop-class framework mapping in the spec header. Gate: regression diff shows exactly the intended ECompensation deltas; classic layer proven unchanged.
  - Measured 2026-07-28: **`ECompensation` is the real faucet and the largest single piece of the flattening**: 2,086 live feedstock reward rows across 96 hunting zones, decomposing to **1,469 record-level ops** (79% of the whole corpus's 1,991 ops). 83 of those zones drop only tiers 2 to 12 and no 94101 today. Only the project-authored slice has a generator (`tools/zone-loot/fill_zone_loot.py`, `tools/iod-loot/generate_iod_loot.py`); the vanilla bulk has none, and whether it is in scope at all is still a scope decision, not a research finding.
  - **R13 contradicts itself on 4 rows and needs a written carve-out.** "Remove direct feedstock drops" and "the classic v31 drop layer stays byte-untouched" cannot both hold: the v31 layer itself drops 94101 on Vekas (npc 1001) and Kugai (npc 1004) in zone 13. Decide and record which rule wins before the spec is authored.
  - **`LimitedDrop.xml` is entirely commented out**: zero live DropGauge blocks, zero live feedstock references. Drops out of scope. Worth remembering for the opposite reason: it is a vanilla-proven guaranteed-floor mechanic that is currently switched off.
...
- **RV-28 Feedstock flattening (R15). DONE.** ~~Research first~~ **research DONE 2026-07-28 (§4.10)**. Remaining: repoint enchant tables across ALL gear tiers onto 94101 (R18), repoint dismantle outputs to grade-scaled quantities of the single item, delete-or-migrate every other reference per R19, fix strings/tooltips. This diverges from the v92 ITEM system (not from v31 content): record it as a systems divergence in the spec header. Gate: enchanting works at a leveling tier and an endgame tier with the single item; no reference to 94102-94112 survives in any live family.
  - **Sizing:** 4,620 real item references across 117 files, decomposing to **1,991 record-level ops**, of which 1,469 are `ECompensation` (RV-26). Everything outside `ECompensation` is about 380 ops. `QuestCompensationData` carries **zero** of them (see §4.10).
  - **Highest-leverage single change:** the whole tier ladder in our own enchant pipeline reduces to one expression, `94100 + rank` (`FEEDSTOCK_BASE_ID` in `tools/enchant-materials/generate_enchant_materials.py`). Pinning it to a constant 94101 and regenerating `specs/patches/002/04-enchant-materials.yaml` replaces 129 hand-written ops with one edit. Two more generators carry tier mappings and need the same treatment: `tools/zone-loot/fill_zone_loot.py` and `tools/iod-loot/generate_iod_loot.py`.
  - **Pre-existing defects the flattening will touch, to fix deliberately rather than by side effect:** 559 endgame items currently enchant with a BOUND Relic material because of that same `94100 + rank` formula, and 216 references point at ids 94119 to 94122, which have no `ItemTemplate` row at all. `MaterialEnchantData` is proven to tolerate those dangling ids (patch 002 deployed with them and the server boots), but a regeneration silently decides their fate either way.
  - **`DecompositionData.xml` is not under spec control.** Rows 206861 to 206868 were committed straight into the server datasheet by hand and are reproducible by no spec; rows 206863 to 206868 are dead (nothing references them). Current fodder yield keys on **SLOT only** (48 for weapon/body, 24 for arm/leg) and ignores grade, which contradicts R15's "grade scales dismantle QUANTITY". Spec-ify the file first, then fix the yield axis.
  - **Four client-carrying families are ABSENT from `ENTITY_SYNC_MAP`** (`eventMatchingEvents`, `itemMixes`, `decompositions`, `achievements`) and have no `sync-config.yaml` descriptor. An absent key is worse than one mapped to `None`: `detect_entities` never sees the block and the client leg is dropped **with no log line**. Adding the map key without the descriptor produces the known `AreaData` failure mode (zero-file plan, prints nothing, exits 0). Both legs are needed.
  - **10 containers collide when the ids collapse** (7 in `Gacha` RandomReward, 3 in `ItemConversion/SeedItem`), merging 29 rows. Their probabilities must be **summed, not dropped**, or the feedstock rate silently doubles. Note that duplicate item ids inside one reward pool are already legal and normal (47 pre-existing `Gacha` duplicate containers): any tooling that "dedups by item id" corrupts them.
  - **`StackAttendanceEvent` has no DSL entity at all** (1 op, server-only, inert): file a `docs/dsl-requests/` entry if R19's delete has to reach it.
...
### 4.10 Corrections from the 2026-07-28 research wave

A seven-report research wave measured the flattening corpus attribute by attribute (full ElementTree walk of 11,410 server files and 23,314 client files, comment-stripped so a commented row is never counted). It corrected several facts this document and earlier passes had recorded. Trust the figures below over any earlier number.

- **`QuestCompensationData` contains ZERO feedstock references.** Zero files, zero quests. The earlier "3,053 references across 70 files" figure is a real count of the wrong family: every one of those hits is in the Relic Fragment / Relic Shard band (94113 to 94118), which R15 explicitly excludes. **RV-28 must not be sized as if quest rewards were involved.** Corollary worth noting: the one family with a fully wired client sync leg is the one family the flattening does not touch.
- **The real corpus is 4,620 references across 117 files**, decomposing to **1,991 record-level ops**, of which **1,469 are `ECompensation` mob loot tables**. The DSL op granularity is the record, not the reference, and no bulk filter operation (`updateWhere`) exists for any reward, material or conversion family.
- **`LimitedDrop.xml` is entirely commented out**: zero live references. Drops out of scope.
- **The whole tier ladder in our own enchant pipeline is one expression**, `94100 + rank`, in `tools/enchant-materials/generate_enchant_materials.py`. `tools/zone-loot/fill_zone_loot.py` and `tools/iod-loot/generate_iod_loot.py` carry the other two tier mappings.
- **Two pre-existing defects sit inside the rows the flattening rewrites:** 559 endgame items enchant with a BOUND Relic material because of that formula, and 216 references point at ids 94119 to 94122, which have no `ItemTemplate` row.
- **`DecompositionData.xml` rows 206861 to 206868 were hand-committed into the server datasheet and are reproducible by no spec.** Rows 206863 to 206868 are dead. The live fodder yield keys on SLOT only (48 weapon/body, 24 arm/leg) and ignores grade, contradicting R15.
- **Four families that carry the feedstock id on the client are absent from `ENTITY_SYNC_MAP`** (`eventMatchingEvents`, `itemMixes`, `decompositions`, `achievements`) and have no `sync-config.yaml` descriptor. An absent key is worse than one mapped to `None`: the client leg is dropped with no log line at all.
- **10 containers collide when the ids collapse** (7 `Gacha`, 3 `ItemConversion`), merging 29 rows; their probabilities must be summed, not dropped.
- **R13 contradicts itself on 4 rows.** It requires removing direct feedstock drops AND leaving the classic v31 layer untouched, but the v31 layer itself drops 94101 on Vekas (1001) and Kugai (1004) in zone 13. A written carve-out is required either way.
- **Research note on the three existing token shops (Kugai 9999008, Bastion of Lok 9999004, Sinestral Manor 9999006): they are item-opened MEDAL_USEABLE right-click shops and function as designed; no NPC is bound to their menus because none is meant to be.** `VillagerMenuItem` binds the ITEM to the menu, so a `reverse_lookup_shop_npcs` miss is the expected result, not a defect. This is the exact pattern ruling R11 rejects, so converting them to physical NPC vendors remains RV-05's job. It is a design change, not a bug, and it is not part of the wave 1 plan.
...
- **Operator / DB, carried over from the retired wave-1 plan (2026-07-31).** Two questions no datasheet can
  answer, both harmless today and both worth asking before release:
  1. **Player stock of the retired feedstock tiers 94102 to 94112.** Character inventories, banks, broker
     listings and mail live in the planet database, which this repo has no access path to and should not
     acquire one. The answer decides whether a reverse conversion ladder (retired tier back to 94101) is
     ever needed. Expected answer pre-release is zero.
  2. **Vanguard per-event on/off state.** `spLoadEventMatchingOff` persists it in the planet database, so the
     datasheet cannot tell whether an operator already switched the system off. It blocked nothing: deleting
     the feedstock reward rows was correct either way.
