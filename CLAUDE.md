...
4. **The datasheet trees are generated output, never a source of value.** Everything dirty in `server_datasheet` and `client_datacenter` must be reproducible by applying the patch. A hand-edit there is only ever a temporary probe used to prove a DSL gap or bug, and it is expected to be destroyed by the next apply. Never pause an apply, a sync, or a deploy to protect manual edits, never ask whether they should be preserved, and never treat them as an acceptance oracle beyond the one comparison they were made for. If a hand-edit encodes something the specs cannot express, that is a DSL request (`docs/dsl-requests/`), not a file to defend. Copying files aside for a diff is fine; treating the copy as work-in-progress is not.
