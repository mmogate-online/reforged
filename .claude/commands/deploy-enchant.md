...
Before executing, ask the user for the patch number (e.g. 001, 002).
...
1. **Generate specs** — run `python generate_enchant_materials.py --patch {patch}` from `tools/enchant-materials/`
2. **Apply enchant-materials** — from `<project_root>`, run: `./dsl.exe apply "reforged/specs/patches/{patch}/enchant-materials.yaml" --path "<server_datasheet>"`
3. **Apply enchant-item-links** — from `<project_root>`, run: `./dsl.exe apply "reforged/specs/patches/{patch}/enchant-item-links.yaml" --path "<server_datasheet>"`
...
Replace `{patch}` with the patch number provided by the user.
