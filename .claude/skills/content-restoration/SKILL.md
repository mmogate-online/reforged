...
### Never write our build order into player-facing text; the gap belongs in the register, not in prose
- **Date/source:** 2026-07-31: live testing of the reward-vector wave found item 95217 "Valkyon Commendation" shipping the tooltip line *"The quartermaster who accepts them has not yet set up."* It had passed `dsl validate`, `migrate` with 0 warnings, a clean client sync, all three pre-deploy gates, a client publish, and a world-server boot. The only thing that caught it was a human reading it in game.
- **Why:** a restoration of this size ships incomplete systems *on purpose*. A currency lands before its vendor, a chain before its final leg, a zone before its padding. That sequencing is correct, and it creates a specific authoring temptation: close the gap in prose so the shipped state does not read as a bug. Spec `002/39`'s own header states that reasoning explicitly ("a currency with nothing to buy otherwise reads as a bug"), which is what makes it a pattern rather than a slip. It is invisible to every automated check because it is valid data describing an invalid subject: **us**. And the wave that creates the gap is never the wave that closes it, so the sentence outlives the excuse for it.
- **Apply:** player-facing text describes the world. It never states what we have not built, what is coming later, or which wave or patch a thing belongs to. Banned in shipped strings: *not yet, coming soon, will be added, for now, temporary, placeholder, TBD, work in progress*, any wave or patch number, and any sentence whose subject is us rather than the world. **Not** banned, and deliberately so: *no longer usable, formerly, obsolete* describe a stable state of the world, and the publisher's own retirement convention uses them (item 447, and the baseline strings for 94111/94112). Record the gap in the zone plan folder and the backlog, where the next session reads it. Codified as `DOCTRINE.md` rule 10.
- **Gate:** `python tools/dc-restore/audit_player_text.py --patch NNN`, exit 0 required, alongside `dungeon_audit.py` and `audit_class_gates.py`. It parses spec YAML so comments never reach it: spec headers may discuss wave order freely, which is exactly where that belongs. It scans specs rather than the datasheet on purpose, because a datasheet scan cannot separate our text from the publisher's 112,000 shipped strings.
- **The wider shape, worth carrying past this one string:** when an incomplete state feels like it needs explaining to the player, that is the signal to check whether it should ship at all, not the signal to write a sentence. If a shipped state cannot be described truthfully without naming the gap, it is not ready.

### Closing a patch removes its content from every future deploy; the dev box's clone must be pulled, not just the payload repo pushed
- **Date/source:** 2026-07-30: testers reported IoD zone quests missing in game, Kishale offering nothing. It read as a bad apply. The dev box's datasheet clone was at an April commit while the local repo was **8 commits ahead**, holding the entire patch-001 restoration baseline. 127 of the 151 files those commits touched were stale or absent on the box, including 27 quest files and the whole Berlon chain. Hours lost.
- **Why:** `deploy_dev.py` sends the **git-dirty set and nothing else**. While a patch is in development its files are dirty and get pushed every deploy; the moment the patch closes and they are committed, they silently drop out of the payload and reach the box only through the box's own clone. If that clone is behind, or the box was reset (`--revert`, or an infra payload redeploy), the content is simply gone. Nothing detects it: `--verify` reported 184 of 184 files hash-verified on every run, because all 184 were exactly the dirty files it knew about. The symptom in game is quest givers with no quests, which looks like a spec failure, not a missing baseline.
- **Apply:** when deployed content does not appear in game, **compare the box's clone HEAD against the local repo HEAD before touching a spec**. `ssh <dev_server_ssh> "git -C <repo> rev-parse --short HEAD"` versus `git -C <server_datasheet> rev-parse --short HEAD`. Different values mean every committed change between them is missing and redeploying cannot fix it. Promotion is TWO steps, not one: push to the payload repo AND pull on the box.
- **When the box cannot reach the git remote,** use `git bundle`. A non-interactive SSH session has no credential store on Windows (GCM's default `wincredman` "does not work over a network/SSH session"), so `git fetch` fails there. `git bundle create` locally, `scp`, then `git fetch <bundle> <branch>:refs/remotes/origin/<branch>` and `merge --ff-only` on the box moves the real commits with no network and leaves genuine history behind. Revert the overlay first (`deploy_dev.py --revert --yes`), because incoming files that are also in the overlay make the merge refuse; then redeploy the overlay on top.
- **Do not read a hash mismatch as corruption.** The box's system gitconfig sets `core.autocrlf=true`, so anything it checks out is CRLF while this tool sftp's LF bytes. After a pull, 126 of 151 files differed byte-for-byte and were identical once line endings were normalised. Compare normalised, or compare with `git status` on the box, which applies the same rules.
