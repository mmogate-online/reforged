...
_Last updated: 2026-07-30 (token architecture wave)_
...
| IoD reward vectors, WAVE 1 CLOSED (specs 34, 35, 37, 38, 39, 40, 41, 42, 43 + amendments to 18, 28, 29, 30; package `progression-tokens`) | ✅ | **Built, applied, gated and deployed 2026-07-30, client `0.1.0-dev.42`. Closed 2026-07-31 by user ruling; the live-validation pass was deliberately SKIPPED** because the only deltas left at that point were tooltip text, which does not justify a restart cycle. Backlog items RV-01, RV-03 (story, zone and repeatable legs), RV-07, RV-08, RV-26 (feedstock slice) and RV-28. **Economy:** feedstock collapses to a single untiered commodity (94101; tiers 94102-94112 retired but RESIDENT so ~4,000 references still resolve), every direct faucet deleted corpus-wide (1,785 `ItemBag`s across 95 zone files, 311 `Gacha` rows, 80 `ItemConversion` rows, 164 Vanguard rows and four smaller families), infusion fodder dismantle becomes grade-scaled (16/48/96 and 8/24/48, rare-anchored so drop removal stays the single supply variable). **Progression:** all 35 live IoD zone quests capped at 25 percent of their bracket's median story quest (XP pool 58,200 to 24,740), and the early-progression token **Valkyon Commendation** (95217) threads into all 63 live IoD quests, story and zone, on an effort-priced ladder capped at 150. A full island clear pays **1,182 tokens to one character**; tokens are unbankable across the reserved band 95214-95217, so income is per character and RV-02 prices against a character-run. Two carve-outs are deliberate and divergence-logged: the classic v31 feedstock drops on Vekas and Kugai stay, and the Kugai token shop keeps selling 94101. **Still no sink until RV-02**, so reward parity is unsatisfied by design. The three wave planning documents were retired 2026-07-31 after their content was verified duplicated at the point of use; `docs/plans/reward-vectors/IOD-BACKLOG.md` is the single surviving document and records where each piece went |
