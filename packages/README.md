...
| `item-ids` | ID Registry (generated) | Per-item constant names (`SLUG_<id>`), demand-driven from specs, sharded by item class. Generated by `tools/item-ids/gen_item_ids.py` from `ItemTemplate*.xml` + `StrSheet_Item*.xml`. Do not hand-edit. | `item-ids/README.md` |
...
| `reforged-loot-bags` | Template (curated) | Parameterized reforged drop-economy `ItemBag` templates (Alkahest, Feedstock, Crystal/Dyad/SmartDyad/Infusion boxes), each on `PROB`/`QTY` params. Derived by `tools/spec-standardize` from the IoD loot table; consumed by `tools/iod-loot/generate_iod_loot.py`. | `reforged-loot-bags/README.md` |
...
| `dyad-crystal-items` | Template (generated) | Shared item-envelope archetypes for all Dyad crystal items: `DyadCrystalCommon` plus per-slot commons and 12 per-tier bases (`Dyad{Slot}{Tier}Base`) that bake category, equipment type, tooltip and tier economics. Source tool: `tools/dyad-crystals/`. Do not hand-edit. | `dyad-crystal-items/README.md` |
